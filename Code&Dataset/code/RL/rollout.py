...
if __name__ == "__main__":
    ...
    model_path = "/datanfs4/wangyan/RL-RAG/WarmUp/sft_cache/Qwen3-0.6B-sft-lora-train-embedding-shot/final-merged"
    ...
    e5_model_path="/datanfs4/wangyan/model_cache/e5-base-v2"
    prompt_file_path = "/datanfs4/wangyan/RL-RAG/WarmUp/prompt.txt"

    RAG_instance = Rollout(model, tokenizer, e5_model_path, prompt_file_path, do_sample = True, max_token_len=100000)

    test_file_path = "/datanfs4/wangyan/RL-RAG/DatasetConstruction/ReconstructedDataset/SplitInfo/test_MusiQue.json"
    # test_file_path = 'train_rl.json'
    with open(test_file_path, "r") as file:
        test_data = json.load(file)
    
    # rollout test in different env

    import concurrent.futures
    import time


    def time_test(max_concurrent, rollout_num):

        rollout_results = {}
        total_start = time.time()  # 总耗时计时

        for idx, item in enumerate(test_data[:5]):
            question_results = [None] * rollout_num
            start_time = time.time()  # 当前 item 耗时计时
            ...
            rollout_results[idx] = question_results
            end_time = time.time()
            print(f"Item {idx} rollout finished in {end_time - start_time:.3f} seconds")

        total_end = time.time()
        print(f"Total rollout time for {len(test_data[:5])} items: {total_end - total_start:.3f} seconds")
    ...
    for i in range(0, 3):
        ...
        rollout_num = 4
        time_test(max_concurrent, rollout_num)        
